...
1. 读取学生作文。
   ...
   - 如果需要调用 Gemini OCR，可先运行 `scripts/gemini_ocr.py`，用 `GEMINI_API_KEY` 通过 OpenAI 兼容接口生成 OCR JSON；多张图片可一次传入并按输入顺序识别为同一篇作文，再基于其中的 `raw_text`、`metadata`、`issues` 和 `warnings` 进入后续批改。
...
当输入是图片或扫描版 PDF 时，可先用 `scripts/gemini_ocr.py` 生成 OCR JSON，再按工作流程继续批改。不要把 API Key 写进文件，使用环境变量传入。
...
- 默认模型可用 `GEMINI_MODEL` 或 `--model` 覆盖；默认接口地址可用 `GEMINI_OPENAI_BASE_URL`、`GEMINI_BASE_URL` 或 `--base-url` 覆盖。
